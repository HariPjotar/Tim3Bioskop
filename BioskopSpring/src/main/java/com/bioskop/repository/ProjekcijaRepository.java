...
public interface ProjekcijaRepository extends JpaRepository<Projekcija, Integer> {
	...
	@Query("SELECT m FROM Mesta m, Rezervacija r, Projekcija p WHERE r.rezervacijaID = m.rezervacija AND p.projekcijaID = r.projekcija AND p.projekcijaID =:id")
	public List<Mesta> vratiPopunjenaMestaUProjekciji(@Param("id") Integer id);
}